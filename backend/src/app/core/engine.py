...
class AgentEngine:
    ...
    async def initialize(self) -> None:
        ...
        async with self._init_lock:
            ...
            logger.info(f"🔌 [ENGINE] Initializing async resources for {self.agent.role}...")
            ...
            logger.info(f"✅ [ENGINE] Initialization complete for {self.agent.role}")
    ...
    async def aclose(self) -> None:
        ...
        async with self._init_lock:
            ...
            logger.info(f"🔌 [ENGINE] Closing resources for {self.agent.role}...")
            ...
            self._initialized = False
            logger.info(f"💤 [ENGINE] Shutdown complete for {self.agent.role}")
        ...
        for i, msg in enumerate(messages):
            if isinstance(msg, ToolMessage):
                ...
                if i > 0 and isinstance(messages[i-1], AIMessage) and messages[i-1].tool_calls:
                    ...
                    logger.warning(f"🧹 Removing orphaned ToolMessage: {msg.content[:50]}...")
    ...
    async def _init_mcp_tools(self, servers: list[str]) -> None:
        ...
        for server_name in servers:
            try:
                ...
                logger.info(f"✅ Connected to MCP Server: {server_name} ({len(server_tools)} tools)")
            except Exception as e:
    ...
    async def _audit_system_health(self) -> str:
        ...
        try:
            ...
            logger.info("🔍 [AUDIT] Checking Sentry for active issues...")
            ...
            logger.warning(f"⚠️ Sentry audit failed: {e}")
        return ""
    ...
    def _build_graph(self) -> "CompiledGraph":
        ...
        async def router(state: AgentState) -> dict[str, object]:
            ...
            logger.info("🧠 [ROUTER] Starting intent classification...")
    ...
    """            
            router_prompt = (
                ...
                "- Questions (cómo, qué, por qué, etc.) = CONVERSATION\n"
                ...
                "### EXAMPLES\n"
                "User: '¿Cómo hago un PR?' → {\"intent\": \"conversation\", \"response\": \"Para hacer un PR...\"}\n"
                "User: 'Crea un archivo test.txt' → {\"intent\": \"task\", \"task_description\": \"Create file test.txt\"}\n"
                "User: 'Haz una página web' → {\"intent\": \"task\", \"task_description\": \"Build a web page\"}\n"
                "User: '¿Qué es Phylactery?' → {\"intent\": \"conversation\", \"response\": \"Phylactery es...\"}\n"
            )

            response = await self.llm_text.ainvoke([SystemMessage(content=router_prompt)] + messages[-5:])
            logger.info(f"🧠 Router raw response: {response.content[:200]}...")
            ...
            try:
                ...
                if json_match:
                    ...
                    if intent == "conversation":
                        ...
                        logger.info(f"🚀 Delegating to Planner: {task_desc}")
            ...
            if any(verb in str(last_human_msg).lower() for verb in action_verbs):
                logger.warning(f"⚠️ Router fallback: Detected action verb, treating as TASK")
                return {
                    "intent": "task",
            ...
            logger.warning(f"⚠️ Router fallback: Treating as CONVERSATION")
            return {
        ...
        async def executor(state: AgentState) -> dict[str, object]:
            ...
            logger.info(f"🦴 [EXECUTOR] Executing step {step_idx + 1}/{len(plan)}: {plan[step_idx] if step_idx < len(plan) else 'N/A'}")
        ...
        async def call_tools(state: AgentState) -> dict[str, list[BaseMessage]]:
            ...
            logger.info("🔧 [TOOLS] Calling MCP tools...")
        ...
        async def finalizer(state: AgentState) -> dict[str, object]:
            ...
            logger.info("✨ [FINALIZER] Synthesizing final response...")
            past_steps = state.get("past_steps", [])
            ...
            logger.info(f"✨ Finalizer generated response: {str(response.content)[:100]}...")
    ...
    async def _compress_history(self, messages: list[BaseMessage]) -> list[BaseMessage]:
        ...
        logger.info(f"💾 [COMPRESSOR] History length ({len(messages)}) exceeds threshold. Summarizing...")
        ...
        try:
            ...
            logger.info("✅ [COMPRESSOR] History compressed successfully.")
